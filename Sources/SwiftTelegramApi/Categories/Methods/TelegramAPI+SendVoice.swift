...
extension TelegramAPI {
    ...
    public func sendVoice(_ input: SendVoiceInput, numberOfAttempts: Int = 1, timeoutInterval: TimeInterval = 60, completionHandler: @escaping (TelegramResult<Message>) -> Void) {
        self.requester.request("sendVoice", object: input, numberOfAttempts: numberOfAttempts, timeoutInterval: timeoutInterval, completion: completionHandler)
    }

    public func sendVoice(_ input: SendVoiceInput, numberOfAttempts: Int = 1, timeoutInterval: TimeInterval = 60) async throws -> Message {
        try await self.requester.request("sendVoice", object: input, numberOfAttempts: numberOfAttempts, timeoutInterval: timeoutInterval)
    }
}
...
public final class SendVoiceInput: IMultiPartFromDataEncodable {
	...
	func encode(_ encoder: MultiPartFromDataEncoder) throws {
		...
		try encoder.append("voice", object: self.voice)
