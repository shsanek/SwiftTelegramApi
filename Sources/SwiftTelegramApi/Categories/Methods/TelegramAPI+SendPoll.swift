...
extension TelegramAPI {
    ...
    public func sendPoll(_ input: SendPollInput, numberOfAttempts: Int = 1, timeoutInterval: TimeInterval = 60) async throws -> Message {
        try await self.requester.request("sendPoll", object: input, numberOfAttempts: numberOfAttempts, timeoutInterval: timeoutInterval)
    }
}
...
public final class SendPollInput: IMultiPartFromDataEncodable {
	...
	func encode(_ encoder: MultiPartFromDataEncoder) throws {
		...
		try encoder.append("question", object: self.question)
		try encoder.append("options", object: self.options)
		try encoder.append("is_anonymous", object: self.isAnonymous)
		try encoder.append("type", object: self.type)
		try encoder.append("allows_multiple_answers", object: self.allowsMultipleAnswers)
		try encoder.append("correct_option_id", object: self.correctOptionId)
		try encoder.append("explanation", object: self.explanation)
		try encoder.append("explanation_parse_mode", object: self.explanationParseMode)
		try encoder.append("explanation_entities", object: self.explanationEntities)
		try encoder.append("open_period", object: self.openPeriod)
		try encoder.append("close_date", object: self.closeDate)
		try encoder.append("is_closed", object: self.isClosed)
