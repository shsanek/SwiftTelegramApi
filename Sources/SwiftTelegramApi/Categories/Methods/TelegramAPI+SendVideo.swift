...
extension TelegramAPI {
    ...
    public func sendVideo(_ input: SendVideoInput, numberOfAttempts: Int = 1, timeoutInterval: TimeInterval = 60) async throws -> Message {
        try await self.requester.request("sendVideo", object: input, numberOfAttempts: numberOfAttempts, timeoutInterval: timeoutInterval)
    }
}
...
public final class SendVideoInput: IMultiPartFromDataEncodable {
	...
	func encode(_ encoder: MultiPartFromDataEncoder) throws {
		...
		try encoder.append("video", object: self.video)
		...
		try encoder.append("supports_streaming", object: self.supportsStreaming)
